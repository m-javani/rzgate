...
pub async fn process_set_room_pkg(
    ...
    let property_id = match payload.get("property_id").and_then(|v| v.as_str()) {
        ...
        _ => return error_response(metrics, RZError::Validation("property_id is required".into())).await,
    };
    let room_type = match payload.get("room_type").and_then(|v| v.as_str()) {
        Some(s) if !s.is_empty() => s,
        _ => return error_response(metrics, RZError::Validation("room_type is required".into())).await,
    };
    ...
    match handler.execute(seg, true, buf).await {
        Ok(field_data) => decode_set_room_pkg_response(metrics, &field_data),
        Err(e) => error_response(metrics, e).await,
    }
}

fn decode_set_room_pkg_response(metrics: MetricsRef, payload: &Bytes) -> Response {
