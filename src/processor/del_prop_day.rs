...
pub async fn process_del_prop_day(
    seg: &str,
    payload: &Value,
    handler: &Handler,
    ...
    match handler.execute(seg, true, buf).await {
        Ok(field_data) => decode_del_prop_day_response(metrics_tx.clone(), &field_data),
        Err(e) => error_response(metrics_tx.clone(), &e.to_string()).await,
    }
}

fn decode_del_prop_day_response(metrics_tx: Sender<MetricsEvent>, payload: &Bytes) -> Response {
